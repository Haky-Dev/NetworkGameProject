...
void RenderManager::Initialize(int windowSizeX, int windowSizeY)
{
...
void RenderManager::CreateVertexBufferObjects()
{
	float rectSize = 0.5f;
	float rect[]
		=
...
int RenderManager::GenPngTexture(char * filePath, GLuint sampling)
{
	//Load Pngs
...
int RenderManager::GenBmpTexture(char * filePath, GLuint sampling)
{
...
bool RenderManager::DeleteTexture(int idx, bool printLog)
{
	if (idx <= 0)
	{
		if (printLog)
...
void RenderManager::AddShader(GLuint ShaderProgram, const char* pShaderText, GLenum ShaderType)
{
	//쉐이더 오브젝트 생성
...
bool RenderManager::ReadFile(char* filename, std::string *target)
{
...
GLuint RenderManager::CompileShaders(char* filenameVS, char* filenameFS)
{
...
void RenderManager::DrawSolidRect(float m_x, float m_y, float z, float size, float r, float g, float b, float a)
{
	//Program select
...
void RenderManager::DrawSolidRect(
	float m_x, float m_y, float z, 
...
void RenderManager::DrawTextureObject(Object* Obj, int textureID)
{
	Vector2d pos = Obj->GetPosition();
	Vector2d volume = Obj->GetVolume();
	...
	if (textureID == -1)
		...
		texID = Obj->GetTexture(textureID);

	if (texID == INVALID_TEXTURE) return;
	 
	DrawTextureRect(pos.m_x * 100, pos.m_y * 100, depth, volume.m_x, volume.m_y, 0, color.r, color.g, color.b, color.a, texID);
}

void RenderManager::DrawTextureRect(
	float m_x, float m_y, float z,
	float sizeX, float sizeY, float sizeZ,
	float r, float g, float b, float a,
	int textureID)
