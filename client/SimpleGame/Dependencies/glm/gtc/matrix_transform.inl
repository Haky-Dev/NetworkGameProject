...
namespace glm
{
	...
	GLM_FUNC_QUALIFIER tmat4x4<T, P> rotate_slow(tmat4x4<T, P> const & m, T angle, tvec3<T, P> const & v)
	{
		...
		Result[0][0] = c + (static_cast<T>(1) - c)      * axis.m_x     * axis.m_x;
		Result[0][1] = (static_cast<T>(1) - c) * axis.m_x * axis.m_y + s * axis.z;
		Result[0][2] = (static_cast<T>(1) - c) * axis.m_x * axis.z - s * axis.m_y;
		Result[0][3] = static_cast<T>(0);

		Result[1][0] = (static_cast<T>(1) - c) * axis.m_y * axis.m_x - s * axis.z;
		Result[1][1] = c + (static_cast<T>(1) - c) * axis.m_y * axis.m_y;
		Result[1][2] = (static_cast<T>(1) - c) * axis.m_y * axis.z + s * axis.m_x;
		Result[1][3] = static_cast<T>(0);

		Result[2][0] = (static_cast<T>(1) - c) * axis.z * axis.m_x + s * axis.m_y;
		Result[2][1] = (static_cast<T>(1) - c) * axis.z * axis.m_y - s * axis.m_x;
...
#		endif
	...
	)
	{
		...
		tmp.m_x = (tmp.m_x - T(viewport[0])) / T(viewport[2]);
		tmp.m_y = (tmp.m_y - T(viewport[1])) / T(viewport[3]);
#		if GLM_DEPTH_CLIP_SPACE == GLM_DEPTH_ZERO_TO_ONE
...
#		endif
	...
	{
		assert(delta.m_x > static_cast<T>(0) && delta.m_y > static_cast<T>(0));
		tmat4x4<T, P> Result(static_cast<T>(1));

		if(!(delta.m_x > static_cast<T>(0) && delta.m_y > static_cast<T>(0)))
			return Result; // Error

		tvec3<T, P> Temp(
			(static_cast<T>(viewport[2]) - static_cast<T>(2) * (center.m_x - static_cast<T>(viewport[0]))) / delta.m_x,
			(static_cast<T>(viewport[3]) - static_cast<T>(2) * (center.m_y - static_cast<T>(viewport[1]))) / delta.m_y,
			static_cast<T>(0));

		// Translate and scale the picked region to the entire window
		Result = translate(Result, Temp);
		return scale(Result, tvec3<T, P>(static_cast<T>(viewport[2]) / delta.m_x, static_cast<T>(viewport[3]) / delta.m_y, static_cast<T>(1)));
	}
