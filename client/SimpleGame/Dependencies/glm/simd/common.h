...
GLM_FUNC_QUALIFIER glm_vec4 glm_vec4_abs(glm_vec4 m_x)
{
	return _mm_and_ps(m_x, _mm_castsi128_ps(_mm_set1_epi32(0x7FFFFFFF)));
}

GLM_FUNC_QUALIFIER glm_ivec4 glm_ivec4_abs(glm_ivec4 m_x)
{
#	if GLM_ARCH & GLM_ARCH_SSSE3_BIT
		return _mm_sign_epi32(x, x);
#	else
		glm_ivec4 const sgn0 = _mm_srai_epi32(m_x, 31);
		glm_ivec4 const inv0 = _mm_xor_si128(m_x, sgn0);
		glm_ivec4 const sub0 = _mm_sub_epi32(inv0, sgn0);
		return sub0;
#	endif
}

GLM_FUNC_QUALIFIER glm_vec4 glm_vec4_sign(glm_vec4 m_x)
{
	glm_vec4 const zro0 = _mm_setzero_ps();
	glm_vec4 const cmp0 = _mm_cmplt_ps(m_x, zro0);
	glm_vec4 const cmp1 = _mm_cmpgt_ps(m_x, zro0);
...
GLM_FUNC_QUALIFIER glm_vec4 glm_vec4_round(glm_vec4 m_x)
{
#	if GLM_ARCH & GLM_ARCH_SSE41_BIT
		return _mm_round_ps(x, _MM_FROUND_TO_NEAREST_INT);
#	else
		glm_vec4 const sgn0 = _mm_castsi128_ps(_mm_set1_epi32(0x80000000));
		glm_vec4 const and0 = _mm_and_ps(sgn0, m_x);
		glm_vec4 const or0 = _mm_or_ps(and0, _mm_set_ps1(8388608.0f));
		glm_vec4 const add0 = glm_vec4_add(m_x, or0);
		glm_vec4 const sub0 = glm_vec4_sub(add0, or0);
		return sub0;
#	endif
}

GLM_FUNC_QUALIFIER glm_vec4 glm_vec4_floor(glm_vec4 m_x)
{
#	if GLM_ARCH & GLM_ARCH_SSE41_BIT
		return _mm_floor_ps(x);
#	else
		glm_vec4 const rnd0 = glm_vec4_round(m_x);
		glm_vec4 const cmp0 = _mm_cmplt_ps(m_x, rnd0);
		glm_vec4 const and0 = _mm_and_ps(cmp0, _mm_set1_ps(1.0f));
...
//roundEven
GLM_FUNC_QUALIFIER glm_vec4 glm_vec4_roundEven(glm_vec4 m_x)
{
	glm_vec4 const sgn0 = _mm_castsi128_ps(_mm_set1_epi32(0x80000000));
	glm_vec4 const and0 = _mm_and_ps(sgn0, m_x);
	glm_vec4 const or0 = _mm_or_ps(and0, _mm_set_ps1(8388608.0f));
...
GLM_FUNC_QUALIFIER glm_vec4 glm_vec4_ceil(glm_vec4 m_x)
{
#	if GLM_ARCH & GLM_ARCH_SSE41_BIT
		return _mm_ceil_ps(x);
#	else
		glm_vec4 const rnd0 = glm_vec4_round(m_x);
		glm_vec4 const cmp0 = _mm_cmpgt_ps(m_x, rnd0);
		glm_vec4 const and0 = _mm_and_ps(cmp0, _mm_set1_ps(1.0f));
...
GLM_FUNC_QUALIFIER glm_vec4 glm_vec4_fract(glm_vec4 m_x)
{
	glm_vec4 const flr0 = glm_vec4_floor(m_x);
	glm_vec4 const sub0 = glm_vec4_sub(m_x, flr0);
	return sub0;
}

GLM_FUNC_QUALIFIER glm_vec4 glm_vec4_mod(glm_vec4 m_x, glm_vec4 m_y)
{
	glm_vec4 const div0 = glm_vec4_div(m_x, m_y);
	glm_vec4 const flr0 = glm_vec4_floor(div0);
	glm_vec4 const mul0 = glm_vec4_mul(m_y, flr0);
	glm_vec4 const sub0 = glm_vec4_sub(m_x, mul0);
	return sub0;
}
...
GLM_FUNC_QUALIFIER glm_vec4 glm_vec4_step(glm_vec4 edge, glm_vec4 m_x)
{
	glm_vec4 const cmp = _mm_cmple_ps(m_x, edge);
	return _mm_movemask_ps(cmp) == 0 ? _mm_set1_ps(1.0f) : _mm_setzero_ps();
}

GLM_FUNC_QUALIFIER glm_vec4 glm_vec4_smoothstep(glm_vec4 edge0, glm_vec4 edge1, glm_vec4 m_x)
{
	glm_vec4 const sub0 = glm_vec4_sub(m_x, edge0);
...
// Agner Fog method
GLM_FUNC_QUALIFIER glm_vec4 glm_vec4_nan(glm_vec4 m_x)
{
	glm_ivec4 const t1 = _mm_castps_si128(m_x);						// reinterpret as 32-bit integer
...
// Agner Fog method
GLM_FUNC_QUALIFIER glm_vec4 glm_vec4_inf(glm_vec4 m_x)
{
	glm_ivec4 const t1 = _mm_castps_si128(m_x);										// reinterpret as 32-bit integer
	glm_ivec4 const t2 = _mm_sll_epi32(t1, _mm_cvtsi32_si128(1));					// shift out sign bit
