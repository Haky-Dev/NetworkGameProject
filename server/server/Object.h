...
public:
	...
	void correctpos(const Object& collider) {
		...
		{
			float len = (m_volume.y + collider.m_volume.y) / 2.0f;
			if (dir.y > 0)
				m_position.y = collider.m_position.y + len;
			else
				m_position.y = collider.m_position.y - len;
		}
		else
		{
			float len = (m_volume.x + collider.m_volume.x) / 2.0f;
			if (dir.x > 0)
				m_position.x = collider.m_position.x + len;
			else
				m_position.x = collider.m_position.x - len;
		}
	}
...
public:
	...
	Player();
	Player(int clientId, int clientSock);
	~Player();
	void Update(float fTimeElapsed);
